...
using FeatureTrackerResulst = Eigen::aligned_map<
        //pnec::features::KeyPointID, Eigen::aligned_vector<std::pair<int, Eigen::Matrix<double, 3, 1>>>>;
        int, Eigen::aligned_vector<std::pair<int, Eigen::Matrix<double, 3, 1>>>>;
...
public:
    ...
    FeaturePerFrame(const Eigen::Matrix<double, 3, 1> &_point, double td) {
        ...
        // velocity.y() = _point(6);
        cur_td = td;
        bv_covariance = Eigen::Matrix3d::Zero();
    }

    Vector3d normalpoint;
    double cur_td;
    Eigen::Matrix3d bv_covariance;
    // Vector2d uv;
    // Vector2d velocity;
    // double z;
    // bool is_used;
